...
LCD::LCD(char pin1, char pin2, char pin3, char pin4, char pin5) {
	clock_pin = pin1;
	mosi_pin = pin2;
	mode_pin = pin3;
	reset_pin = pin4;
...
void LCD::clearScreen() {
	digitalWrite(mode_pin, data_mode);
...
void LCD::sendCommands(char *commands) {
	...
	for (int i = 0; commands[i]; i++)
		shiftOut(mosi_pin, clock_pin, MSBFIRST, commands[i]);
	digitalWrite(enable_pin, HIGH);
}
void LCD::sendData(char *data) {
	...
	for (int i = 0; data[i]; i++)
		shiftOut(mosi_pin, clock_pin, MSBFIRST, data[i]);
	digitalWrite(enable_pin, HIGH);
}
void LCD::sendDatum(char datum) {
	digitalWrite(mode_pin, data_mode);
	digitalWrite(enable_pin, LOW);
	shiftOut(mosi_pin, clock_pin, MSBFIRST, datum);
	digitalWrite(enable_pin, HIGH);
}
...
void LCD::sendScreen(char data[84][6]) {
	this->sendCommands("\x80\x40");
	digitalWrite(mode_pin, data_mode);
	digitalWrite(enable_pin, LOW);
	for (int i = 0; i < 84; i++)
		for (int j = 0; j < 6; j++)
			shiftOut(mosi_pin, clock_pin, MSBFIRST, data[i][j]);
	digitalWrite(enable_pin, HIGH);
}
